...
public class CoordinateConversion
{
  ...
  public double getC(double x,double y,double m)
  {
  	//s+="input received ="+m;
  	//s+="lol"+Math.tan(deg2rad(m))+"<br>";
  	return (y-(m)*x); 
  }
  ...
  public double LatLngDistance(double latf,double lonf,double latt,double lont) {
	  double er = 6366.707;

	  double latFrom = degreeToRadian(latf);
	  double latTo   = degreeToRadian(latt);
	  double lngFrom = degreeToRadian(lonf);
	  double lngTo   = degreeToRadian(lont);
...
//      UTM = longZone + " " + latZone + " " + ((int) _easting) + " "          + ((int) _northing);
  ...
  public double getAngle(double x1,double y1,double x2,double y2){
		return (radianToDegree(Math.atan((y2-y1)/(x2-x1))));
	}
}
